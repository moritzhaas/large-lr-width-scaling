# %%
"""
Compute spectral norms of convolutional layers.
Copied from https://github.com/blaisedelattre/lip4conv/blob/main/bounds.py
"""
...
def estimate(X, n=32, n_iter=100, name_func="ryu19", return_time=False,device="cuda"):
    ...
    if name_func == "delattre2024":
        ...
        sigma = compute_araujo2021(X, n_iter=n_iter, return_time=return_time,device=device)
    elif name_func == "singla2021":
        sigma = compute_singla2021(X, n_iter=n_iter, return_time=return_time,device=device)
...
# my implementation for CIFAR10 and ResNets:
def compute_conv_spectral_norm(model, name,device="cuda"):
    with torch.no_grad():
        for thisname, thisparam in model.named_parameters():
            if thisname == name:
                param = thisparam.clone().detach()
                out_channels, in_channels, H, W = param.shape
                # CIFAR10 and our ResNets:
                input_size = int(32*64/in_channels) # at this layer, input image has H and W input_size
                bound = estimate(param,input_size,100,"ryu2019",return_time=False,device=device).item()
                return bound
    
    
# from utils.resnet_mup import ResNet18
# toyresnet = ResNet18(wm=1,parameterization='mup', perturbation='mpp',variant='sam',out_multiplier = 0.25,ll_zero_init=True)
# for name, param in toyresnet.named_parameters():
#     if 'conv' in name:
#         print(compute_conv_spectral_norm(toyresnet,name,"cpu"))


# # %%
# # for standard ResNet18 on ImageNet:
...
#               "delattre2023long",
# ]
# model_resnet_18 = models.resnet18(pretrained=True).eval() #.cuda().eval()
...
#                                             inp_shape[0],
#                                             n_iter_name[name],
#                                             name if "long" not in name else name[:-4],
#                                             return_time=True,
#                                             device="cpu")
#                 bound = bound.item()
#                 #print(name, "conv weight dim", param.shape, "n", inp_shape[0], "bound", bound, "time", curr_time)
#                 print(name, param.shape, "n", inp_shape[0], "bound", bound, "time", curr_time)
#                 # if is_downsample:
...
# # araujo 2021 is always much larger than the other methods -> dont use araujo
# # singla2021 slightly larger than ryu and delattre, but which one is close to the truth???

# # delattre2023 says ryu19 is arbitrarily exact, but takes long to converge. but delattre2023 has longer compute time?!
# # ryu19 with n_iter=100 looks good enough and faster than delattre2023!
